    ...
    return (
        <div className="rounded-md border border-white/10 bg-white/5">
            <Button
                ...
                onClick={() => setOpen(o => !o)}
                variant="ghost"
                className="w-full flex items-center justify-between px-3 py-2 text-left text-sm font-medium tracking-wide text-white/80 hover:text-white hover:bg-white/10"
            >
            ...
            {open && (
                <div className="px-4 pb-4 pt-2 text-2xs leading-relaxed space-y-5 max-h-[480px] overflow-auto investor-scroll">
                    <section>
                        <h3 className="text-xs font-medium text-teal-300 mb-2">ASX MVA NFT - Investor Overview</h3>
                    </section>
                    <section className="space-y-2">
                        <h4 className="font-medium text-white/80 text-2xs flex items-center gap-1">Financial Highlights <span className="text-white/40">(YE 2024 vs YE 2023)</span></h4>
                        <div className="overflow-auto rounded border border-white/10 bg-white/[0.03]">
                            <table className="w-full text-2xs min-w-[420px]">
                    ...
                    <div className="pt-2 border-t border-white/10 text-center text-2xs space-y-1">
...
function SectionTitle({ title }: { title: string }) {
    return <h4 className="font-medium text-white/80 text-2xs">{title}</h4>;
}
