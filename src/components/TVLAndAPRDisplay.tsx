 ...
 */
const TVLAndAPRDisplay: React.FC<TVLAndAPRDisplayProps> = ({
  poolId,
  onUpdateTVL,
  isVisible = true,
}) => {
  ...
  const calculateLPPrice = useCallback(() => {
    if (pool?.type === "lp" && reserve0 && reserve1 && totalSupply) {
      ...
      const price1 = prices[token1.address.toLowerCase()]
        ? BigInt(Math.round(prices[token1.address.toLowerCase()] * 1e18))
        : BigInt(0);
      const price2 = prices[token2.address.toLowerCase()]
        ? BigInt(Math.round(prices[token2.address.toLowerCase()] * 1e18))
        : BigInt(0);
      const totalReserveValue =
        (reserve1 * price1 + reserve0 * price2) / BigInt(1e18);
  ...
  useEffect(() => {
    ...
    if (rewardData) {
      ...
      const apy =
        (Math.pow(1 + apr / compoundingEffect, compoundingEffect) - 1) * 100; // Calculate APY
