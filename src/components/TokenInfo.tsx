...
const TokenInfo = () => {
  ...
  const formatPrice = (price) => {
    ...
    return (
      <span>
        {mainPart}.<span>{firstTwoDecimals}</span>
        <span className={styles.smallDecimals}>{lastTwoDecimals}</span>
      </span>
    );
  };
  ...
  const priceChangeClass =
    priceChange < 0 ? styles.red : priceChange > 0 ? styles.green : "";
  ...
  return (
    <div className={styles.InfoList}>
      ...
      <p className={styles.tokenInfoItem}>
        24h Vol: ${tokenData.market_data.total_volume.usd.toFixed(2)}
      </p>
    </div>
  );
};
