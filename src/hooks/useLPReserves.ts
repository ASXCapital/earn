...
const useLPReserves = (lpContractAddress: string) => {
  ...
  // Fetching reserves
  const { data: reservesData, isError: reservesError, isLoading: reservesLoading } = useReadContract({
  ...
  useEffect(() => {
    if (reservesData && !reservesError && !reservesLoading && token0Data && token1Data) {
